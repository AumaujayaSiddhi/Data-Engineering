...
public class FindKFrequentItemsetMapper extends Mapper<Object, Text, Text, IntWritable>{
	...
	protected void setup(Mapper<Object, Text, Text, IntWritable>.Context context)
        ...
        Iterator<int[]> k_sized_combinations_of_unique_items = CombinatoricsUtils.combinationsIterator(list_of_remaining_unique_items.size(), 2);
        while (k_sized_combinations_of_unique_items.hasNext()) {
			...
			for (int i = 0; i < next_combination.length; i++) {
				combinationString.append(list_of_remaining_unique_items.get(next_combination[i]) + ((i<next_combination.length-1) ? ",":"") );
			}
